# Pyares Simple Device Template
## Introduction
This is a template for a a simple, single file [PyAres](https://github.com/AFRL-ARES/PyAres) devicce service. This approach is best for getting started with PyAres or if your device protocol is relatively simple and doesn't require a lot of custom logic that isn't already abstracted away by external libraries. This template repository also includes a basic sketch for an Ardiuno Uno that can be used a device service on real hardware if so desired.

*N.B.* Because they interact with the real world, PyAres device services tend to be the most complicated services to write. You should make sure you have a firm grasp on communicating with your device using just python before trying to implement a device service. 
...
3. Edit `start_simple_device.py` to customize your device.
